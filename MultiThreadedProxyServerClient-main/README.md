...
*(Placeholder for your UML diagram image - upload it to your repo and update the link)*
![Proxy Server UML Diagram](https://github.com/Garvit240205/HTTP-Proxy-Server/MultiThreadedProxyServerClient-main/pics/image.png)
...
*   **Networking:** Standard BSD Sockets API (`socket`, `bind`, `listen`, `accept`, `connect`, `send`, `recv`).
...
If you prefer to run the server directly on a Linux system (or WSL on Windows).
...
**Important Note regarding Caching:** The current project builds and runs the proxy *with* the caching feature enabled by default (`proxy_server_with_cache.c`).

## Demo

*(Placeholder for your Cache Demo image - upload it to your repo and update the link)*
